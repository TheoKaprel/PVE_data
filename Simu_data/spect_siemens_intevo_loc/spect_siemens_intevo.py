...
def add_collimator_lehr(sim, head, debug):
    ...
    if debug:
        size = [1, 30, 20]
    tr = [0, 1.27 * mm, 2.1997 * mm]
    rot = Rotation.from_euler("y", 90, degrees=True).as_matrix()
    offset = [0, 0.3175 * mm * 2, 0.549925 * mm * 2]
...
def add_collimator_melp(sim, head, debug):
    ...
    if debug:
        size = [1, 30, 20]
    tr = [0, 4.08 * mm, 7.066767 * mm]
    rot = Rotation.from_euler("y", 90, degrees=True).as_matrix()
    offset = [0, 1.02 * mm * 2, 1.76669 * mm * 2]
    repeat_colli_hole(sim, hole, size, tr, rot, offset)
...
def add_digitizer_v2(sim, crystal_name, name):
    # create main chain
    mm = gate.g4_units.mm
...
def add_digitizer_v3(sim, crystal_name, name):
    # create main chain
    mm = gate.g4_units.mm
    digitizer = Digitizer(sim, crystal_name, name)
...
def set_head_orientation(head, collimator_type, radius, gantry_angle=0):
    ...
    r = Rotation.from_euler("xz", (180, 90 + gantry_angle), degrees=True)
    head.translation = r.apply([distance, 0, 0])
    head.rotation = r.as_matrix()
    return r

def set_head_orientation_yrot(head, collimator_type, radius, gantry_angle=0):
    # pos is the distance from entrance detection plane and head boundary
    pos, _, _ = compute_plane_position_and_distance_to_crystal(collimator_type)
    distance = radius - pos
    # rotation X180 is to set the detector head-foot
    # rotation Y90 is the gantry angle
    r = Rotation.from_euler("xy", (270,90 + gantry_angle), degrees=True)
    head.translation = r.apply([distance, 0, 0])
    head.rotation = r.as_matrix()
    return r
...
def create_simu_for_arf_training_dataset(
    sim, colli_type, max_E, activity, rr, channels, radius=None
):
    ui = sim.user_info
    mm = gate.g4_units.mm
    cm = gate.g4_units.cm
    ...
    digit = add_digitizer_v2(sim, crystal.name, "digit")
    ...
    arf.energy_windows_actor = ew.name
