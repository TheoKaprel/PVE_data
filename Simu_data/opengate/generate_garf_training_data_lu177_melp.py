...
def main():
    ...
    detector_plane.size = [1 * nm, 57 * cm,44 * cm ]
    detector_plane.translation = [pos-1*mm,0,0]
    ...
    s1.position.radius = 57.6 * cm/2  # FIXME why ???
    s1.position.translation = [-20 * cm,0,0]
    s1.position.rotation = Rotation.from_euler('y', 90, degrees=True).as_matrix()
    s1.direction.type = "iso"
    s1.energy.type = "range"
    s1.energy.min_energy = 0.150 * MeV
    s1.energy.max_energy = 0.300 * MeV
    ...
    arf.energy_windows_actor = f"{spect.name}_energy_window"
    arf.russian_roulette = args.rr
