...
parser.add_argument('--grad_act',action ="store_true", help = "if --grad_act, activity in background/ROIs/lesions are made heterogeneous")
...
def generate(opt):
    ...
    for n in range(opt.nb_data):
        ...
        if with_attmaps:
            attmap_4mm = itk.imread(attmap_ref.replace('.mhd', '_4mm.mhd'), pixel_type=pixelType)
            if opt.attmapaugmentation:
            ...
            attmap_4mm_np = itk.array_from_image(attmap_4mm)
            vSpacing_recpfp = np.array(attmap_4mm.GetSpacing())
            vSize_recfp = np.array(attmap_4mm_np.shape)[::-1]
            vOffset_recfp = np.array(attmap_4mm.GetOrigin())
        else:
        ...
        if opt.save_src:
            ...
            #         ref="src_nograd", dtype=dtype)


            src_img_normedToTotalCounts_4mm = gatetools.applyTransformation(input=src_img_normedToTotalCounts, like=attmap_4mm, spacinglike=None, matrix=None, newsize=None,
        ...
        if opt.project:
            if opt.lesion_mask:
                ...
                lesion_mask_4mm = gatetools.applyTransformation(input=lesion_mask_img,
                                                                                like=attmap_4mm, spacinglike=None,
                ...
                save_me(img=lesion_mask_4mm,ftype=opt.type, output_folder = opt.output_folder, src_ref = source_ref,
                        ref = "lesion_mask_4mm", dtype = np.uint16)
                ...
                save_me(array=lesion_mask_fp_array,ftype=opt.type, output_folder = opt.output_folder, src_ref = source_ref,
                        ref = "lesion_mask_fp", dtype = np.uint16, img_like = output_forward_lesion_mask)
            ...
            if with_attmaps:
                ...
                save_me(array=noisy_projection_array, ftype=opt.type, output_folder=opt.output_folder, src_ref=source_ref,
                        ref="PVE_att_noisy", dtype=np.uint16, img_like=output_forward_PVE)
                ...
                save_me(array=noisy_projection_array, ftype=opt.type, output_folder=opt.output_folder, src_ref=source_ref,
                        ref="PVE_noisy", dtype=np.uint16, img_like=output_forward_PVE)
            ...
            if opt.rec_fp>0:
                ...
                forward_projector_rec_fp_att.SetInput(2, attmap_4mm)
